...
public class TokenService {
    ...
    public void saveUserToken(String token, Long userId) {
        ...
        stringRedisTemplate.opsForValue().set(key, userId.toString(), tokenExpireHours, TimeUnit.HOURS);
    ...
    public void saveAdminToken(String token, Long adminId) {
        ...
        stringRedisTemplate.opsForValue().set(key, adminId.toString(), tokenExpireHours, TimeUnit.HOURS);
    ...
    public void refreshToken(String token) {
        ...
        if (Boolean.TRUE.equals(userExists)) {
            stringRedisTemplate.expire(userKey, tokenExpireHours, TimeUnit.HOURS);
            return;
        }

        Boolean adminExists = stringRedisTemplate.hasKey(adminKey);
        if (Boolean.TRUE.equals(adminExists)) {
            stringRedisTemplate.expire(adminKey, tokenExpireHours, TimeUnit.HOURS);
        }
    }
}